...
## Fluxos Principais
- **Aluno**: Realiza login/cadastro na rota `/login`. O formulário conta com a opção de exibir/ocultar a senha digitada para facilitar o preenchimento. O fluxo de cadastro é otimizado sem exigência de confirmação de e-mail por padrão (se desativado no Supabase), logando o usuário imediatamente. Há também um fluxo de **Esqueci a Senha** com envio de link de recuperação para definir nova senha na rota `/reset-password` (que agora é isolada e protegida, impedindo que usuários acessem diretamente por dentro do painel estando logados normalmente). Só enxerga cursos liberados na tabela `permissoes`. O player do YouTube roda na rota `/curso/:id` e salva o progresso na tabela `progresso` e acumula o tempo da sessão atual na tabela `progresso_diario`. Na tela de aula, há um widget minimalista e interativo de **Meta Diária do Curso** com barra de progresso em tempo real que enche a cada 5 segundos enquanto o vídeo roda. Ao bater a meta na sessão atual, a borda do player (seja de vídeo or leitura de PDF) recebe um brilho verde/cinza sutil por 4 segundos, um Toast no canto inferior da tela é exibido e o streak ("foguinho") é updated uma única vez no banco. Se o aluno já iniciou a aula com a meta cumprida no dia, a barra de progresso permanece em 100% com o distintivo "Meta Batida! 🏆" sem re-disparar efeitos incômodos. A navegação das aulas é feita pelo acordeão inferior. A aula ativa em reprodução é destacada e, ao finalizar uma aula, a próxima aula é reproduzida automaticamente. Se houver um PDF de material de apoio atrelado à aula, é disponibilizado um botão para download. Há uma rota de Dashboard em `/stats` com gráficos interativos e lista vertical de acessos recentes. Também há a rota de **Metas e Cronogramas** em `/metas`, onde o usuário pode definir minutos de estudo diários por curso. Esta página possui gamificação informando dias seguidos de meta ("foguinho") e quanto falta para bater a meta de estudo no dia atual.
...
- **Segurança da Redefinição de Senha**: A rota `/reset-password` é protegida contra acessos diretos maliciosos ou acidentais de dentro do painel. Uma flag de sessão temporária (`is_resetting_password`) é salva no `sessionStorage` no carregamento inicial apenas se a URL contiver `#type=recovery` ou no evento `PASSWORD_RECOVERY` do Supabase Auth. O formulário de redefinição de senha exige essa flag ativa no `sessionStorage`; caso contrário, o usuário é redirecionado para `/` (se logado) ou `/login` (se deslogado). A flag é removida imediatamente do `sessionStorage` após a redefinição com sucesso da senha. A funcionalidade de trocar de senha por dentro do painel para usuários logados normalmente foi totalmente desativada.
...
- **Serviço de E-mail (Resend SMTP)**: O envio de e-mails transacionais (boas-vindas, confirmação e recuperação de senha) é delegado ao SMTP da API do Resend (`smtp.resend.com`) configurado com a chave do projeto (`re_LDkAAesa_812V6YjCoPbG5fCy3imZv5jo`), fornecendo entregabilidade estável e templates HTML/CSS premium customizados.
